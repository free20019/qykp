  ...
  var vm = new Vue({
    ...
    methods: {
      ...
      handleAddClick() {
        ...
        layer.open(config.layerOptions('出差申请'))
      ...
      handleManagerClick() {
        ...
        layer.open(config.layerOptions('出差申请-经理审核'))
        $('#dialogEdited').html(html)
        util.wrapEl('#dialogEdited', 'md6')
      },
      handleFinanceClick() {
        var html = this.createFormHtml('财务')
        layer.open(config.layerOptions('出差申请-财务审核'))
        $('#dialogEdited').html(html)
        util.wrapEl('#dialogEdited', 'md6')
      },
      handleDepartmentClick() {
        var html = this.createFormHtml('部门')
        layer.open(config.layerOptions('出差申请-部门审核'))
        $('#dialogEdited').html(html)
        util.wrapEl('#dialogEdited', 'md6')
      },
      handleDetailsClick() {
        var html = this.createFormHtml('查看')
        layer.open(config.layerOptions('出差申请-详情'))
        $('#dialogEdited').html(html)
        util.wrapEl('#dialogEdited', 'md6')
      },
      createFormHtml(type) {
